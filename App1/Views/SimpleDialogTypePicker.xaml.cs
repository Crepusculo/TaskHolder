...
namespace App1.Views
{
    ...
    /// </summary>
    public sealed partial class SimpleDialogTypePicker : ContentDialog
    {
        ...
        public SimpleDialogTypePicker(ref UserInfo user, ref TaskDataModel model, Grid root)
        {
            this.model = model;
            this.user = user;
            ...
            TypeComboBox.PlaceholderText = model.Type;

        }


        public SimpleDialogTypePicker(ref UserInfo user, ref GoalDataModel model, Grid root)
        {
            this.model = model;
            this.user = user;
            ...
            ContentDialogButtonClickEventArgs args)
        {
            try
            {
                ...
                {
                    ((GoalDataModel) model).Type = ((StrWrap)TypeComboBox.SelectedItem).str;
                    ...
                    TextBlock userGroupTextBlock = DebugUtil.FindControl<TextBlock>(_root, typeof(TextBlock),
                        "TypeTextBlock");
                    userGroupTextBlock.Text = ((GoalDataModel) model).Type;
                }
                else if (model.GetType() == typeof(TaskDataModel))
                {
                    ((TaskDataModel) model).Type = ((StrWrap)TypeComboBox.SelectedItem).str;
                    DebugUtil.WriteLine(this, model.ToString());
                    NetworkUtil.UpdateTaskDataModel(user.Username, user.Token, (TaskDataModel) model);
                    TextBlock userGroupTextBlock = DebugUtil.FindControl<TextBlock>(_root, typeof(TextBlock),
                        "TypeTextBlock");
                    userGroupTextBlock.Text = ((TaskDataModel) model).Type;
                }
            }
            catch (Exception)
            {
